...
exports.neo4jSchema = exports.codeMirrorSettings = undefined;

var _functions = __webpack_require__(0);

var _functions2 = _interopRequireDefault(_functions);

var _procedures = __webpack_require__(1);
...
var codeMirrorSettings = exports.codeMirrorSettings = {
...
var neo4jSchema = exports.neo4jSchema = {
  consoleCommands: [{ name: ':clear' }, { name: ':play' }, { name: ':help', description: 'this is help command' }, {
    name: ':server',
    commands: [{
      name: 'user',
      commands: [{ name: 'list', description: 'listdesc' }, { name: 'add' }]
    }]
  }, { name: ':schema' }, { name: ':history' }, { name: ':queries' }],
  labels: [':Legislator', ':State', ':Party', ':Body', ':Bill', ':Subject', ':Committee', ':Congress'],
  relationshipTypes: [':REPRESENTS', ':IS_MEMBER_OF', ':ELECTED_TO', ':PROPOSED_DURING', ':SPONSORED_BY', ':VOTED_ON', ':REFERRED_TO', ':SERVES_ON', ':DEALS_WITH'],
  propertyKeys: ['bioguideID', 'code', 'name', 'type', 'billID', 'title', 'thomasID', 'birthday', 'wikipediaID', 'currentParty', 'state', 'votesmartID', 'fecIDs', 'republicanCount', 'otherCount', 'cspanID', 'democratCount', 'lastName', 'firstName', 'party', 'opensecretsID', 'icpsrID', 'religion', 'lisID', 'govtrackID', 'gender', 'district', 'number', 'enacted', 'officialTitle', 'vetoed', 'active', 'popularTitle', 'cosponsor', 'vote', 'jurisdiction', 'url', 'rank', 'washpostID'],
  functions: _functions2.default.data.map(function (data) {
    return {
      name: data.row[0],
      signature: data.row[1].replace(data.row[0], '')
    };
  }),
  procedures: _procedures2.default.data.map(function (data) {
    var name = data.row[0];
    var signature = data.row[1].replace(data.row[0], '');

    var returnItems = [];
    var matches = signature.match(/\([^)]*\) :: \((.*)\)/i);

    if (matches) {
      returnItems = matches[1].split(', ').map(function (returnItem) {
        var returnItemMatches = returnItem.match(/(.*) :: (.*)/);
        return {
